...
class MatchesSpider(Spider):
    ...
    def start_requests(self):
        start_url = 'http://www.betexplorer.com/next/soccer/'
    ...
    def parse(self, response):
        items = []
        competitions = response.xpath('//table[@class="table-matches js-nrbanner-t"]//tbody')
        for c in competitions:
            item = Match()
            item['area_name'], item['competition_name'] = \
                    c.xpath('./tr[@class="js-tournament"]//a/text()').extract_first().split(':')
